...
def normalize(series: pd.Series, mean: float | None = None, std: float | None = None) -> pd.Series:
    """
...
def pre_process_features(df: pd.DataFrame, run_config: RunConfig, pre_process_categorial: bool = True) -> pd.DataFrame:
    ...
    for var in run_config.input_variables:
        if var in constants.LN_NORM_VARS:
            scaled_input_df[var], scale_params[var] = normalize(np.log1p(df[var]), *run_config.get_scale_params(var))
        elif var in constants.NORM_VARS:
            scaled_input_df[var], scale_params[var] = normalize(df[var], *run_config.get_scale_params(var))
        elif var in constants.MIN_MAX_SCALE_PARAMS:
            scaled_input_df[var] = min_max_scale(df[var], var)
        elif var == constants.InputVariable.Geomorphon:
...
def pre_process_vs30(values: np.ndarray):
